...
class TimeProfiler:
    ...
    def end_step(self, print_stats: bool = None):
        ...
        if print_stats is None:
            print_stats = (self.step_count % self.print_freq == 0) and (
    ...
    def print_step_stats(self):
        ...
        if "total_step" in stats:
            ...
            print(f"\n📊 Overall Metrics:")
            print(
                f"  • Step Time: {step_time*1000:.2f}ms (±{stats['total_step']['std']*1000:.2f}ms)"
            )
        ...
        if self.detailed and len(stats) > 1:
            ...
            for name, timing in sorted_stats:
                ...
                percentage = (
                    timing["mean"] / stats.get("total_step", {"mean": 1})["mean"]
                ) * 100
                ...
                print(
                    f"  {name:20s}: {mean_time:7.2f}ms ±{std_time:6.2f}ms [{bar}] {percentage:5.1f}%"
                )
        ...
        if self.track_memory and self.memory_stats:
            ...
            if "cuda_allocated_step_end" in self.memory_stats:
                ...
                print(
                    f"  GPU Memory: {cuda_alloc:.2f}GB allocated / {cuda_reserved:.2f}GB reserved"
                )
