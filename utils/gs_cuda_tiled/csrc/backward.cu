...
__global__ void nd_rasterize_backward_kernel(
  ...
  __shared__ float rgbs_batch[MAX_BLOCK_SIZE * MAX_CHANNELS];
  ...
  for (int b = 0; b < num_batches; ++b) {
    ...
    if (idx < range.y) {
      ...
      for (int c = 0; c < channels; ++c)
        rgbs_batch[tr * channels + c] = rgbs[channels * g_id + c];
    }
    ...
    for (int t = 0; t < batch_size; ++t) {
      ...
      if (valid) {
        ...
        const float *rgb = &rgbs_batch[t * channels];
...
__global__ void nd_rasterize_backward_no_tiles_kernel(
    const dim3 img_size, const unsigned channels,
    const float2 *__restrict__ xys, const float3 *__restrict__ conics,
    const float *__restrict__ rgbs, const float *__restrict__ v_output,
    float2 *__restrict__ v_xy, float3 *__restrict__ v_conic,
    float *__restrict__ v_rgb, int *__restrict__ pixel_topk) {
  ...
  for (int b = 0; b < num_batches; ++b) {
    ...
    for (int t = 0; t < batch_size; ++t) {
      ...
      if (valid) {
        ...
        // update v_sigma for this gaussian
        const float v_sigma =
            (rgb.x * v_out.x + rgb.y * v_out.y + rgb.z * v_out.z) * (-d);
        // update v_conic for this gaussian
        v_conic_local = {0.5f * v_sigma * delta.x * delta.x,
                         v_sigma * delta.x * delta.y,
                         0.5f * v_sigma * delta.y * delta.y};
