...
class Attention(nn.Module):
    def __init__(
        ...
        self.attn_drop = attn_dropout_p
        self.proj_drop = nn.Dropout(resid_dropout_p)
...
class SelfDecoder(nn.Module):
    def __init__(
        ...
        self.ffn = FeedForward(dim=dim, ffn_dropout_p=ffn_dropout_p)
...
class Decoder_Decoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, q: torch.Tensor, freqs_cis: torch.Tensor = None):
        ...
        k, v = map(
            lambda t: rearrange(t, "b n (h d) -> b n h d", h=self.n_head), (k, v)
        )
