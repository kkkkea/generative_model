...
void check_block_size(const int block_h, const int block_w) {
  TORCH_CHECK(block_h > 0 && block_w > 0, "BLOCK_H and BLOCK_W must be positive");
  TORCH_CHECK(block_h * block_w <= MAX_BLOCK_SIZE,
              "BLOCK_H * BLOCK_W must be <= ", MAX_BLOCK_SIZE);
}
...
compute_cov2d_bounds_tensor(const int num_pts, torch::Tensor &covs2d) {
  CHECK_INPUT(covs2d);
  const at::cuda::OptionalCUDAGuard device_guard(device_of(covs2d));
...
std::tuple<torch::Tensor, torch::Tensor> map_gaussian_to_intersects_tensor(
    ...
    const torch::Tensor &radii, const torch::Tensor &cum_tiles_hit,
...
torch::Tensor get_tile_bin_edges_tensor(int num_intersects, int num_tiles,
                                        const torch::Tensor &isect_ids_sorted) {
  CHECK_INPUT(isect_ids_sorted);
  const at::cuda::OptionalCUDAGuard device_guard(device_of(isect_ids_sorted));
...
rasterize_forward_tensor(const std::tuple<int, int, int> tile_bounds,
  ...
  CHECK_INPUT(colors);
  const at::cuda::OptionalCUDAGuard device_guard(device_of(xys));
  check_block_size(std::get<1>(block), std::get<0>(block));
...
std::tuple<torch::Tensor> nd_rasterize_forward_tensor(
    const std::tuple<int, int, int> tile_bounds,
    const std::tuple<int, int, int> block,
    const std::tuple<int, int, int> img_size,
    const torch::Tensor &gaussian_ids_sorted, const torch::Tensor &tile_bins,
    const torch::Tensor &xys, const torch::Tensor &conics,
    const torch::Tensor &colors) {
  CHECK_INPUT(gaussian_ids_sorted);
  CHECK_INPUT(tile_bins);
...
nd_rasterize_backward_tensor(
    const unsigned img_height, const unsigned img_width, const unsigned BLOCK_H,
    const unsigned BLOCK_W, const torch::Tensor &gaussians_ids_sorted,
  ...
  CHECK_INPUT(colors);
  const at::cuda::OptionalCUDAGuard device_guard(device_of(xys));
  check_block_size(BLOCK_H, BLOCK_W);
  ...
  const dim3 tile_bounds = {(img_width + BLOCK_W - 1) / BLOCK_W,
                            (img_height + BLOCK_H - 1) / BLOCK_H, 1};
...
std::tuple<torch::Tensor, // out_img
...
nd_rasterize_forward_topk_norm_tensor(
  ...
  torch::Tensor out_img = torch::zeros({img_height, img_width, channels},
                                       xys.options().dtype(torch::kFloat32));
  torch::Tensor pixel_topk = torch::zeros({img_height, img_width, TOP_K},
                                          xys.options().dtype(torch::kInt32));
...
std::tuple<torch::Tensor, // out_img
           torch::Tensor  // pixel_topk
           >
nd_rasterize_forward_no_tiles_tensor(const std::tuple<int, int, int> img_size,
  ...
  nd_rasterize_forward_no_tiles<<<grid_size_dim3, block_dim3>>>(
      img_size_dim3, channels, num_points,
      (float2 *)xys.contiguous().data_ptr<float>(),
      (float3 *)conics.contiguous().data_ptr<float>(),
      colors.contiguous().data_ptr<float>(),
      out_img.contiguous().data_ptr<float>(),
...
nd_rasterize_backward_no_tiles_tensor(
  ...
  nd_rasterize_backward_no_tiles_kernel<<<grid_size, block_size>>>(
      img_size, channels, (float2 *)xys.contiguous().data_ptr<float>(),
      (float3 *)conics.contiguous().data_ptr<float>(),
      colors.contiguous().data_ptr<float>(),
      v_output.contiguous().data_ptr<float>(),
      (float2 *)v_xy.contiguous().data_ptr<float>(),
...
std::tuple<torch::Tensor, // out_img
           torch::Tensor  // pixel_topk
           >
nd_rasterize_forward_simple_tensor(const std::tuple<int, int, int> img_size,
                                   const unsigned num_points,
                                   const torch::Tensor &xys,
...
nd_rasterize_backward_simple_tensor(
    const unsigned img_height, const unsigned img_width,
    const torch::Tensor &xys, const torch::Tensor &scale,
    const torch::Tensor &rot, const torch::Tensor &feat,
    const torch::Tensor &v_output, // dL_dout_color
    const torch::Tensor &pixel_topk) {
  CHECK_INPUT(xys);
  CHECK_INPUT(scale);
...
rasterize_backward_tensor(const unsigned img_height, const unsigned img_width,
                          const unsigned BLOCK_H, const unsigned BLOCK_W,
...
project_gaussians_2d_scale_rot_forward_tensor(
    const int num_points, torch::Tensor &means2d, torch::Tensor &scales2d,
    torch::Tensor &rotation, const unsigned img_height,
    const unsigned img_width, const std::tuple<int, int, int> tile_bounds) {
  CHECK_INPUT(means2d);
  CHECK_INPUT(scales2d);
  ...
  project_gaussians_2d_scale_rot_forward_kernel<<<
      ...
      num_points, (float2 *)means2d.contiguous().data_ptr<float>(),
      (float2 *)scales2d.contiguous().data_ptr<float>(),
      (float *)rotation.contiguous().data_ptr<float>(), img_size_dim3,
      tile_bounds_dim3,
      // Outputs.
  ...
  CUDA_CALL(cudaDeviceSynchronize());
  return std::make_tuple(xys_d, radii_d, conics_d, num_tiles_hit_d);
}
...
project_gaussians_2d_scale_rot_backward_tensor(
    const int num_points, torch::Tensor &means2d, torch::Tensor &scales2d,
    torch::Tensor &rotation, const unsigned img_height,
    const unsigned img_width, torch::Tensor &radii, torch::Tensor &conics,
    torch::Tensor &v_xy, torch::Tensor &v_conic) {
  CHECK_INPUT(means2d);
  CHECK_INPUT(scales2d);
  CHECK_INPUT(rotation);
  CHECK_INPUT(radii);
