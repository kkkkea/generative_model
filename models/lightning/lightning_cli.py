...
class ReWriteRootDirCli(LightningCLI):
    ...
    def add_default_arguments_to_parser(self, parser: LightningArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            type=bool,
            default=True,
            help=("Enable per-run random seed (synced across ranks)"),
        )
    ...
    def find_latest_checkpoint(self, checkpoint_dir: str) -> Union[str, None]:
        ...
        if not ckpts_with_steps:
            ...
            checkpoint_files = [
                ckpt
                for _, ckpt in sorted(
                    ckpts_with_steps, key=lambda x: x[0], reverse=True
                )
            ]
    ...
    def instantiate_trainer(self, **kwargs: Any) -> Trainer:
        ...
        if auto_resume and not is_resume and self.subcommand == "fit":
            ...
            if latest_ckpt:
                ...
                if os.path.exists(default_root_dir) and os.listdir(default_root_dir):
                    logger.warning(
                        f"Directory {default_root_dir} exists but no checkpoint found. Starting fresh training anyway due to auto_resume=True"
                    )
                else:
                    logger.info(
                        "No checkpoint found for auto-resume, starting fresh training"
                    )
        ...
        if os.path.exists(default_root_dir) and "debug" not in default_root_dir:
            if (
                ...
                raise FileExistsError(
                    f"{default_root_dir} already exists. Use --auto_resume=true to automatically resume, or specify --ckpt_path to resume from a specific checkpoint"
                )
        ...
        if enable_per_run_seed:
            ...
            def _wait_for_file(
                path: str, timeout_s: float = 120.0, interval_s: float = 0.1
            ) -> bool:
                start = time.time()
            ...
            if rank_int == 0:
                try:
                    ...
                    self.run_seed = int(
                        hashlib.sha256(str(mix).encode("utf-8")).hexdigest(), 16
                    ) % (2**31 - 1)
                except Exception:
                ...
                if _wait_for_file(seed_file):
                    try:
                        ...
                        logger.info(
                            f"Rank {rank_int} loaded run seed {self.run_seed} from {seed_file}"
                        )
                    except Exception:
                        self.run_seed = None
                if self.run_seed is None:
                    ...
                    self.run_seed = int(time.time()) % (2**31 - 1)
                    logger.warning(
                        f"Rank {rank_int} using fallback run seed {self.run_seed}"
                    )
        else:
            logger.info(
                "per_run_seed disabled; skipping per-run seed generation/seeding"
            )
    ...
    def fit(self, **kwargs) -> None:
        ckpt_path = getattr(self, "auto_resume_ckpt_path", None) or self._get(
            self.config_init, "ckpt_path", default=None
        )
